...
def run_clustering(
    ...
    if auto_k:
        log("开始自动识别聚类数量 (原理: K=25 预聚类 + 相近颜色合并)...")
        # 【算法详解：如何自动识别颜色数量？】
        # 1. 假设图像里最多有 25 种颜色，先用 KMeans 找出 25 个中心点。
        # 2. 遍历这 25 个中心点，如果两个颜色的 LAB 距离非常近（小于 12），
        #    说明它们在视觉上几乎一样，就把它们合并。
        # 3. 最终剩下的独立颜色数量，就是自动识别出的最优 k 值。
        ...
        merge_threshold = 12.0  # LAB 颜色空间的差异阈值
        merged_centers = []
        for c in centers:
...
with gr.Blocks(
    ...
    btn_cluster.click(
        ...
        inputs=[
            img_input,
            cb_auto_k, sl_clusters, sl_threshold, sl_blur, sl_min_area,
