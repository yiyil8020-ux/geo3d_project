...
#
# median_filter = 中值滤波函数
# 【什么是中值滤波？】
# 对数组中的每个元素，取它周围一个方形窗口内所有值的中位数作为新值。
# 【为什么用它？】
# KMeans 聚类的结果是一张标签图（每个像素是 0~k-1 的整数），
# 边界处会有锯齿（像素在两个标签间反复跳动）。
# 对标签图做中值滤波，能让边界处的少数派像素被多数派覆盖，消除锯齿。
# 【为什么不用高斯模糊？】
# 高斯模糊取平均值，对整数标签来说平均值无意义（标签 0 和 2 的平均=1，但 1 可能是另一种地层）。
# 中值滤波取中位数，结果一定是某个已有的标签，语义正确。
...
median_size_str = input("请输入中值滤波窗口大小（必须奇数，推荐 5~15，默认 7，越大边界越平滑）：").strip()
median_size = int(median_size_str) if median_size_str else 7
# 【什么是中值滤波（Median Filter）？】
# 对图像中的每一个像素，看它周围一个正方形窗口内的所有像素值，
# 把这些值从小到大排序，取正中间那个值（即"中位数"）作为新值。
#
# 举例：窗口大小 = 7，即 7x7 = 49 个像素
# 假设某个边界像素周围有 30 个属于地层 A、19 个属于地层 B
# 排序后中位数是地层 A 的标签 -> 这个像素被归入地层 A
#
# 【为什么它能消除锯齿？】
# KMeans 逐像素分类时，边界处的像素可能因为颜色微小差异被反复分到不同类别，
# 形成 A-B-A-B-A 的锯齿。中值滤波会让每个像素"服从多数"，
# 把锯齿处的少数派像素强制归入多数派，边界就变平滑了。
#
# 【它和高斯模糊有什么区别？】
# 高斯模糊取"平均值"，会产生中间值（比如标签 0 和 2 平均得到 1，这是错的）。
# 中值滤波取"中位数"，结果一定是某个已有的标签值，不会凭空产生新标签。
# 所以中值滤波是对标签图（整数图）做平滑的正确方法。
#
# 窗口越大 -> 边界越平滑，但小地层可能被"吞掉"
# 窗口越小 -> 保留更多细节，但锯齿消除不够

smooth_size_str = input("请输入 mask 边缘平滑核大小（必须奇数，推荐 7~15，默认 9，越大越平滑）：").strip()
smooth_size = int(smooth_size_str) if smooth_size_str else 9
# 【这个参数做什么？】
# 在每个地层的二值 mask（黑白图）上再做一次"高斯模糊 + 重新二值化"来柔化边缘。
#
# 具体过程：
#   原始 mask：边界是硬的（一边是 0，一边是 255，中间没有过渡）
#         | 高斯模糊（核大小 = smooth_size）
#   模糊后：边界变成渐变（0 -> 50 -> 128 -> 200 -> 255）
#         | 二值化：> 127 的变成 255，<= 127 的变成 0
#   结果：边界还是硬的，但原来的锯齿尖角被"削圆"了
#
# 【为什么中值滤波之后还需要这一步？】
# 中值滤波作用在 label_img 上（所有地层一起处理），效果是全局性的。
# 但每个单独的 mask 提取出来后，边缘还可能有像素级别的毛刺。
# 这一步是对单个 mask 的精细化打磨。
#
# smooth_size 越大 -> 边缘越圆滑，但拐角细节丢失越多
# smooth_size 越小 -> 保留更多细节，但平滑效果有限
...
print(f"  中值滤波窗口:   {median_size}")
print(f"  边缘平滑核:     {smooth_size}")
...
# ==================== 8.5 对 label_img 做中值滤波（边界平滑 - 优化 1） ====================
# 【这一步解决什么问题？】
# KMeans 聚类是逐像素独立决策的：每个像素只看自己的颜色，不管旁边是什么。
# 这导致两种地层的交界处出现锯齿 -- 像素在两个类别之间反复跳动。
# 中值滤波让每个像素"参考邻居的意见"，少数服从多数，从而消除锯齿。
#
# 【具体做了什么？】
# 对 label_img（形状 h*w 的整数数组，每个值是 0 ~ n_clusters-1）做中值滤波。
# 窗口大小 = median_size * median_size
# 对于窗口内的所有标签值，取中位数（出现次数最多的那个大致等于中位数）。
#
# 【举例：median_size = 7】
# 边界处某个像素（标签=3）周围 7*7=49 个像素中：
#   标签 2 出现 28 次，标签 3 出现 21 次
#   中位数 = 2 -> 这个像素被改成标签 2
# 效果：边界不再锯齿，而是平滑的曲线。

print(f"\n正在对 label_img 做中值滤波（窗口 {median_size}*{median_size}）...")

label_img_before = label_img.copy()
# 保留一份滤波前的备份，用于统计有多少像素被修改

label_img = median_filter(label_img, size=median_size)
# median_filter(输入数组, size=窗口大小)
# 来自 scipy.ndimage 模块
# size=7 表示用 7*7 的正方形窗口
# 返回值：和输入同样形状的数组，每个位置的值是窗口内的中位数
#
# 【为什么用 scipy 的 median_filter 而不是 cv2.medianBlur？】
# cv2.medianBlur 只支持 uint8（0~255），而我们的 label_img 是 int32 类型。
# scipy 的 median_filter 支持任意数据类型。

changed_pixels = np.sum(label_img != label_img_before)
changed_pct = changed_pixels / (h * w) * 100
print(f"  中值滤波修改了 {changed_pixels:,} 个像素 ({changed_pct:.1f}%)")
print(f"  -> 这些主要是边界处的锯齿像素和小碎片。")

# 保存中值滤波后的 label_img 用于调试
np.save(os.path.join(debug_dir, "label_img_median.npy"), label_img)
...
for i in range(n_clusters):
    ...
    # ---- 清理步骤 4：高斯模糊 + 重新二值化（边缘精细平滑 - 优化 2） ----
    # 【这一步做了什么？】
    # 经过前面的处理，mask 的边缘虽然比原始聚类干净很多，但仍然是"硬边缘"：
    # 相邻两个像素可能一个是 0（黑），一个是 255（白），中间没有过渡。
    # 这种硬边缘在放大看时仍然有像素级别的毛刺和棱角。
    #
    # 解决方法分两步：
    #
    # 第一步：对 mask 做高斯模糊
    #   原来的硬边缘（0 | 255）变成了渐变（0 -> 50 -> 128 -> 200 -> 255）
    #   这相当于让边缘变成一条"模糊的过渡带"
    #
    # 第二步：用阈值 127 重新切回黑白
    #   > 127 的像素 -> 255（白，属于这个地层）
    #   <= 127 的像素 -> 0（黑，不属于这个地层）
    #   过渡带中，原来属于凸出的锯齿尖角的像素，模糊后值降到 127 以下 -> 被切掉
    #   原来属于凹进的锯齿缺口的像素，模糊后值升到 127 以上 -> 被填上
    #   最终效果：锯齿被削平，边缘变成平滑的曲线
    #
    # 【形象理解】
    # 想象你用粗毛笔描了一个边缘不整齐的形状（= 原始 mask）
    # 然后用手指把墨迹的边缘抹开（= 高斯模糊）
    # 再用一条清晰的线在模糊边缘的中间重新画一条分界线（= 阈值 127 二值化）
    # 新画出来的分界线就是平滑的。

    mask_uint8 = cv2.GaussianBlur(mask_uint8, (smooth_size, smooth_size), 0)
    # GaussianBlur(输入图, (核宽, 核高), sigma)
    # 对 mask 的 0/255 灰度图做高斯模糊
    # sigma=0 表示让 OpenCV 根据核大小自动计算合适的标准差
    # 模糊后边缘区域的值变成 0~255 之间的渐变

    mask_uint8 = (mask_uint8 > 127).astype(np.uint8) * 255
    # mask_uint8 > 127 -> 布尔数组（True/False）
    # .astype(np.uint8) -> 转成 0/1
    # * 255 -> 转成 0/255
    # 这就完成了"重新二值化"：渐变的边缘被切回了清晰但平滑的边界
